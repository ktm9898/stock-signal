...
def test_strat(name, buy_fn, sell_fn, sl=10, tp=20):
    total_trades = 0
    win_trades = 0
...
# 1. Reversal Breakout with Volume Filter & Early Entry (ADX >= 30, -DI cross below ADX, 거래량 >= 120%, RSI <= 55)
def r1_buy(a, i):
    p_m = a[i-1, 11] if i > 0 else a[i, 11]
    p_a = a[i-1, 9] if i > 0 else a[i, 9]
    return a[i, 9] >= 30 and p_m > p_a and a[i, 11] <= a[i, 9] and a[i, 8] >= 120 and a[i, 4] <= 55
def r1_sell(a, i): return a[i, 4] >= 65

# 2. +DI Golden Cross Reversal (+DI cross above -DI, ADX >= 25, 거래량 >= 120%, RSI <= 60, 볼린저 0.3~0.7)
def r2_buy(a, i):
    p_p = a[i-1, 10] if i > 0 else a[i, 10]
    p_m = a[i-1, 11] if i > 0 else a[i, 11]
    return p_p <= p_m and a[i, 10] > a[i, 11] and a[i, 9] >= 25 and a[i, 8] >= 120 and (0.3 <= a[i, 5] <= 0.7)
def r2_sell(a, i): return a[i, 4] >= 65

# 3. Enhanced Strategy 1 (ADX >= 30, -DI cross below ADX, 거래량 >= 100%, 볼린저 %b <= 0.7, 손절 -10%, 익절 +20%)
def r3_buy(a, i):
    p_m = a[i-1, 11] if i > 0 else a[i, 11]
    p_a = a[i-1, 9] if i > 0 else a[i, 9]
    return a[i, 9] >= 30 and p_m > p_a and a[i, 11] <= a[i, 9] and a[i, 8] >= 100 and a[i, 5] <= 0.70
def r3_sell(a, i): return a[i, 4] >= 65

res = [
    test_strat('0. 전략 1번 (기존 순정)', lambda a, i: a[i, 9] >= 30 and (a[i-1, 11] if i>0 else a[i, 11]) > (a[i-1, 9] if i>0 else a[i, 9]) and a[i, 11] <= a[i, 9], lambda a, i: a[i, 4] >= 65, sl=20, tp=None),
    test_strat('1. [추세전환 돌파 v1] ADX>=30, -DI 하향돌파, 거래량>=120%, RSI<=55 | RSI>=65 | 손절-10%, 익절+20%', r1_buy, r1_sell, sl=10, tp=20),
    test_strat('2. [DI 골든크로스 돌파] +DI 상향돌파, ADX>=25, 거래량>=120%, 볼린저 0.3~0.7 | RSI>=65 | 손절-10%, 익절+20%', r2_buy, r2_sell, sl=10, tp=20),
    test_strat('3. [스마트 추세돌파 1번 진화형] ADX>=30, -DI 하향돌파, 거래량>=100%, 볼린저<=0.70 | RSI>=65 | 손절-10%, 익절+20%', r3_buy, r3_sell, sl=10, tp=20)
]
