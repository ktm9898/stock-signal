...
def run_eval(name, buy_fn, sell_fn, stop_loss=20, take_profit=None, cooldown=0):
    total_trades = 0
    win_trades = 0
    ...
    return {
        ...
        'profit_factor': round(float(pf := profit_factor), 2),
        'avg_return': round(float(avg_ret), 2),
        'avg_days': round(float(avg_days), 1),
        'avg_holdings': round(float(avg_holdings), 1),
        'max_holdings': max_holdings,
        'net_profit_million': round(float(net_profit_krw / 1000000), 1),
        'return_on_avg_cap': round(float(return_on_avg_cap), 2)
    }
...
# 2. Strategy 1 (고승률 스나이퍼 v2)
def s1_buy(a, i): return a[i, 4] <= 24 and a[i, 5] <= 0.05
def s1_sell(a, i): return a[i, 4] >= 55

bench_no_cd = run_eval("전략 1번 (쿨다운 0일 / 없음)", b1_buy, b1_sell, stop_loss=20, take_profit=None, cooldown=0)
bench_cd10 = run_eval("전략 1번 (쿨다운 10일)", b1_buy, b1_sell, stop_loss=20, take_profit=None, cooldown=10)
s1_no_cd = run_eval("고승률 스나이퍼 (쿨다운 0일 / 없음)", s1_buy, s1_sell, stop_loss=10, take_profit=15, cooldown=0)
s1_cd10 = run_eval("고승률 스나이퍼 (쿨다운 10일)", s1_buy, s1_sell, stop_loss=10, take_profit=15, cooldown=10)

print("\n" + "="*95)
for s in [bench_no_cd, bench_cd10, s1_no_cd, s1_cd10]:
    print(f"[{s['name']}]")
    print(f"  - 승률: {s['win_rate']}% | 손익비: {s['profit_factor']} | 평균수익률: {s['avg_return']}% | 체결: {s['total_trades']}회 | 평잔: {s['avg_holdings']}개 (최대 {s['max_holdings']}개) | 평잔대비수익률: {s['return_on_avg_cap']}%\n")
print("="*95)
