import json
import numpy as np

with open('data/stocks_350_real.json', encoding='utf-8') as f:
    d = json.load(f)

raw_data = d['preloaded_data']
START_DATE = '2021-01-04'
END_DATE = '2026-08-20'
FEE_RATE = 0.0025

stock_arrays = []
stock_dates = []
all_dates_set = set()

for ticker, rows in raw_data.items():
    if ticker in ['069500', 'KOSPI', 'KOSDAQ']: continue
    sub = [r for r in rows if START_DATE <= r[0] <= END_DATE]
    if len(sub) < 10: continue
    dates = [r[0] for r in sub]
    for dt in dates: all_dates_set.add(dt)
    arr = np.zeros((len(sub), 12), dtype=np.float32)
    for i, r in enumerate(sub):
        arr[i, 0] = r[1] or 0 # open
        arr[i, 1] = r[2] or 0 # high
        arr[i, 2] = r[3] or 0 # low
        arr[i, 3] = r[4] or 0 # close
        arr[i, 4] = r[9] if r[9] is not None else 50 # rsi
        arr[i, 5] = r[10] if r[10] is not None else 0.5 # bb_pct
        arr[i, 6] = r[12] if r[12] is not None else 50 # stoch_k
        arr[i, 7] = r[13] if r[13] is not None else 100 # disparity20
        arr[i, 8] = r[14] if r[14] is not None else 100 # volume_ratio (5-day)
        arr[i, 9] = r[6] if r[6] is not None else 0 # adx
        arr[i, 10] = r[7] if r[7] is not None else 0 # plus_di
        arr[i, 11] = r[8] if r[8] is not None else 0 # minus_di
    stock_arrays.append(arr)
    stock_dates.append(dates)

sorted_sim_dates = sorted(list(all_dates_set))
num_sim_dates = len(sorted_sim_dates)
date_to_idx = {dt: i for i, dt in enumerate(sorted_sim_dates)}

def test_trend_strategy(adx_min, plus_min, bb_min, rsi_min, rsi_max, vol_min, exit_rsi, stop_loss=10, take_profit=20):
    total_trades = 0
    win_trades = 0
    total_profit_krw = 0.0
    total_loss_krw = 0.0
    total_ret = 0.0
    total_days = 0
    active_daily = np.zeros(num_sim_dates, dtype=np.int32)
    trade_amount = 1000000
    
    for s_idx in range(len(stock_arrays)):
        arr = stock_arrays[s_idx]
        dates = stock_dates[s_idx]
        n_rows = len(arr)
        in_pos = False
        entry_price = 0.0
        entry_bar_idx = 0
        
        for i in range(n_rows - 1):
            if not in_pos:
                # Trend Conditions
                if arr[i, 9] < adx_min: continue # ADX >= adx_min
                if arr[i, 10] < plus_min: continue # +DI >= plus_min
                if arr[i, 10] <= arr[i, 11]: continue # +DI > -DI
                if arr[i, 5] < bb_min: continue # bb_pct >= bb_min
                if not (rsi_min <= arr[i, 4] <= rsi_max): continue # rsi in range
                if vol_min is not None and arr[i, 8] < vol_min: continue # volume_ratio >= vol_min
                
                if arr[i + 1, 0] > 0:
                    in_pos = True
                    entry_price = arr[i + 1, 0] * (1.0 + FEE_RATE)
                    entry_bar_idx = i + 1
            else:
                curr_close = arr[i, 3]
                paper_return = ((curr_close - entry_price) / entry_price) * 100.0
                is_exit = False
                if stop_loss is not None and paper_return <= -stop_loss: is_exit = True
                elif take_profit is not None and paper_return >= take_profit: is_exit = True
                elif arr[i, 4] >= exit_rsi: is_exit = True
                    
                if is_exit and arr[i + 1, 0] > 0:
                    exit_price = arr[i + 1, 0] * (1.0 - FEE_RATE)
                    ret = ((exit_price - entry_price) / entry_price) * 100.0
                    h_days = max(1, (i + 1) - entry_bar_idx)
                    total_trades += 1
                    gain = trade_amount * (ret / 100.0)
                    if ret >= 0:
                        win_trades += 1
                        total_profit_krw += gain
                    else:
                        total_loss_krw += abs(gain)
                    total_ret += ret
                    total_days += h_days
                    d_start = date_to_idx.get(dates[entry_bar_idx], 0)
                    d_end = date_to_idx.get(dates[i + 1], num_sim_dates - 1)
                    active_daily[d_start:d_end + 1] += 1
                    in_pos = False
                    
        if in_pos:
            exit_price = arr[-1, 3] * (1.0 - FEE_RATE)
            ret = ((exit_price - entry_price) / entry_price) * 100.0
            h_days = max(1, (n_rows - 1) - entry_bar_idx)
            total_trades += 1
            gain = trade_amount * (ret / 100.0)
            if ret >= 0:
                win_trades += 1
                total_profit_krw += gain
            else:
                total_loss_krw += abs(gain)
            total_ret += ret
            total_days += h_days
            d_start = date_to_idx.get(dates[entry_bar_idx], 0)
            d_end = date_to_idx.get(dates[-1], num_sim_dates - 1)
            active_daily[d_start:d_end + 1] += 1
            
    if total_trades < 50: return None
    win_rate = win_trades / total_trades * 100.0
    pf = total_profit_krw / total_loss_krw if total_loss_krw > 0 else 99.9
    avg_ret = total_ret / total_trades
    avg_days = total_days / total_trades
    avg_holdings = float(np.mean(active_daily))
    max_holdings = int(np.max(active_daily))
    net_profit = total_profit_krw - total_loss_krw
    avg_cap = max(1, avg_holdings) * trade_amount
    ret_cap = net_profit / avg_cap * 100.0 if avg_cap > 0 else 0
    
    return {
        'params': (adx_min, plus_min, bb_min, rsi_min, rsi_max, vol_min, exit_rsi, stop_loss, take_profit),
        'trades': total_trades,
        'win_rate': round(float(win_rate), 2),
        'pf': round(float(pf), 2),
        'avg_ret': round(float(avg_ret), 2),
        'avg_days': round(float(avg_days), 1),
        'avg_holdings': round(float(avg_holdings), 1),
        'max_holdings': max_holdings,
        'ret_cap': round(float(ret_cap), 2)
    }

print("Running Trend Strategy Grid Search...")
candidates = []

for adx_m in [25, 30, 35]:
    for p_m in [25, 30]:
        for bb_m in [0.5, 0.7, 0.85]:
            for r_min, r_max in [(50, 65), (55, 70), (50, 60)]:
                for v_m in [100, 150, None]:
                    for e_r in [70, 75]:
                        for sl in [8, 10, 12]:
                            for tp in [15, 20, 25, None]:
                                res = test_trend_strategy(adx_m, p_m, bb_m, r_min, r_max, v_m, e_r, sl, tp)
                                if res and res['avg_holdings'] <= 35:
                                    candidates.append(res)

print(f"Total valid trend strategies found: {len(candidates)}")

# Filter top candidates
top_pf = sorted(candidates, key=lambda x: (x['pf'], x['win_rate']), reverse=True)[:5]
top_win = sorted(candidates, key=lambda x: (x['win_rate'], x['pf']), reverse=True)[:5]
top_ret = sorted(candidates, key=lambda x: (x['ret_cap'], x['pf']), reverse=True)[:5]

print("\n" + "="*95)
print("🏆 [TOP 3 최적 추세 돌파/모멘텀 전략 (평잔 10~30개 내외)]")
print("="*95)

# Select best balanced 3
best_curated = [
    # 1. 고승률 주도주 돌파 (ADX>=30, +DI>=25, +DI>-DI, 볼린저>=0.7, 거래량>=100 | RSI>=75 or +20%, 손절-10%)
    test_trend_strategy(30, 25, 0.7, 55, 70, 100, 75, 10, 20),
    # 2. 강력한 상단 돌파 (ADX>=30, +DI>=30, 볼린저>=0.85, 거래량>=150 | RSI>=75 or +25%, 손절-8%)
    test_trend_strategy(30, 30, 0.85, 55, 70, 150, 75, 8, 25),
    # 3. 추세 눌림목 돌파 (ADX>=25, +DI>=25, 볼린저>=0.5, 거래량>=100 | RSI>=70 or +15%, 손절-10%)
    test_trend_strategy(25, 25, 0.5, 50, 60, 100, 70, 10, 15)
]

labels = [
    "1. [고승률 주도주 돌파형] ADX>=30, +DI>=25, +DI>-DI, 볼린저>=0.7, RSI 55~70, 거래량>=100 | 익절: RSI>=75 or +20% | 손절: -10%",
    "2. [불꽃 랠리 상단 돌파형] ADX>=30, +DI>=30, +DI>-DI, 볼린저>=0.85, RSI 55~70, 거래량>=150 | 익절: RSI>=75 or +25% | 손절: -8%",
    "3. [스마트 추세 눌림목형] ADX>=25, +DI>=25, +DI>-DI, 볼린저>=0.5, RSI 50~60, 거래량>=100 | 익절: RSI>=70 or +15% | 손절: -10%"
]

for name, r in zip(labels, best_curated):
    if r:
        print(f"[{name}]")
        print(f"  - 승률: {r['win_rate']}% | 손익비: {r['pf']} | 건당평균수익: {r['avg_ret']}% | 5년체결: {r['trades']}회 | 평잔: {r['avg_holdings']}개 (최대 {r['max_holdings']}개) | 평잔대비 누적수익률: {r['ret_cap']}%\n")
print("="*95)
